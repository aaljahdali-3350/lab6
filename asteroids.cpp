...
public:
	Game() {
		...
		for (int j=0; j<10; j++) {
			...
			a->color[0] = (Flt)(rnd()*255)/255.0;
			a->color[1] = (Flt)(rnd()*255)/255.0;
			a->color[2] = (Flt)(rnd()*255)/255.0;
		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					Flt xdir = cos(rad);
					Flt ydir = sin(rad);
					b->pos[0] += xdir*OFFSETTED_BY;
					b->pos[1] += ydir*OFFSETTED_BY;
					b->vel[0] += xdir*(OFFSETTED_BY/4.0f) + rnd()*0.1;
					b->vel[1] += ydir*(OFFSETTED_BY/4.0f) + rnd()*0.1;
					b->color[0] = 1.0f;
					b->color[1] = 1.0f;
					b->color[2] = 1.0f;
					++g.nbullets;
	...
	if (e->type == MotionNotify) {
		if (savex != e->xbutton.x || savey != e->xbutton.y) {
			...
			if (xdiff > 0) {
				...
				if (g.ship.angle >= MAX_ANGLE_DEGREE)
					g.ship.angle -= MAX_ANGLE_DEGREE;
			}
			else if (xdiff < 0) {
				g.ship.angle += 0.05f * (float)xdiff;
...
void buildAsteroidFragment(Asteroid *ta, Asteroid *a)
{
	...
	// ta->color[1] = 0.8;
	// ta->color[2] = 0.7;
	ta->color[0] = (Flt)(rnd()*255)/255.0;
	ta->color[1] = (Flt)(rnd()*255)/255.0;
	ta->color[2] = (Flt)(rnd()*255)/255.0;
